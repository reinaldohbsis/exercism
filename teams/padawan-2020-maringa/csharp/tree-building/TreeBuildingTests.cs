...
public class TreeBuildingTests
{
    ...
    public void Unbalanced_tree()
    {
        var records = new[]
        {
            new TreeBuildingRecord { RecordId = 5, ParentId = 2 }, // resposta ao comentario 2
            new TreeBuildingRecord { RecordId = 3, ParentId = 2 }, // resposta ao comentario 2
            new TreeBuildingRecord { RecordId = 2, ParentId = 0 }, // comentario 2
            new TreeBuildingRecord { RecordId = 4, ParentId = 1 }, // resposta ao comentario 1
            new TreeBuildingRecord { RecordId = 1, ParentId = 0 }, // comentario 1
            new TreeBuildingRecord { RecordId = 0, ParentId = 0 }, // post
            new TreeBuildingRecord { RecordId = 6, ParentId = 2 }  // resposta ao cometnario 2
        };
    ...
    private static void AssertTreeIsBranch(Tree tree, int id, int childCount)
    {
        ...
        Assert.False(tree.IsLeaf); // esperando que seja falso
        Assert.Equal(childCount, tree.Children.Count); // quantos filhos ele tem
    }

    private static void AssertTreeIsLeaf(Tree tree, int id) // Folha não tem filho
    {
        Assert.Equal(id, tree.Id);
